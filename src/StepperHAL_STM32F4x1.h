...
struct mm { float value; explicit mm(float v) : value(v) {} };
struct step { int32_t count; explicit step(int32_t s) : count(s) {} };
struct Speed { float rpm; explicit Speed(float r) : rpm(r) {} };
struct Feed { float mmPerMin; explicit Feed(float f) : mmPerMin(f) {} };
struct mms2 { float value; explicit mms2(float a) : value(a) {} };
...
class StepperHAL {
public:
    StepperHAL(TIM_TypeDef* timer, uint32_t channel,
    ...
    void setManualJerk(float jerkAccUp, float jerkAccFlat, float jerkAccDown,
    ...
    inline void useSCurve(bool enable) {
        (void)enable;
        Serial.print("[StepperHAL] ("); Serial.print(getLabel()); Serial.print(") ");
...
protected:
    ...
    void computeDefaultJerk(float accelMMs2, float decelMMs2,
...
inline bool StepperHAL::hasBacklash() const {
    return (_useBacklashStep && _backlashStep.count > 0) ||
           (_useBacklashMM && _backlashMM.value > 0.0f);
}
